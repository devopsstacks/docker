...
if [[ -z "${PM_MSSQL}" ]];then
    ...
    if  [ -f /etc/php.d/*mssql.ini] ; then
        ...
        if [ "${PM_MSSQL}" == "install" ];then
            yum install -y freetds php56-mssql ;
        fi
    fi
fi
...
if [[ -z "${PM_ODBC}" ]];then
    ...
    if  [ -f /etc/php.d/*odbc.ini] ; then
        ...
        if [ "${PM_ODBC}" == "install" ];then
            yum install -y unixODBC unixODBC-devel php56-odbc ;
        fi
    fi
fi
...
if [[ -z "${PM_OCI}" ]];then
    ...
    if [ -x "$(which pecl)" ]; then
        ...
        yum install -y httpd24 php56-devel php56-pecl-apcu gcc libaio ;
        cd /tmp ;
        if [ "${PM_OCI}" == "10" ];then
